...
def download_data() -> int:
    ...
    utils.create_bucket(bucket_name, client)
    utils.write_to_store(bucket_name, features_train, 'features_train', client)
    utils.write_to_store(bucket_name, target_train, 'target_train', client)
    utils.write_to_store(bucket_name, features_test, 'features_test', client)
    utils.write_to_store(bucket_name, target_test, 'target_test', client)
...
def gen_hyperparam_grid() -> int:
    ...
    utils.write_to_store(bucket_name, grid, 'hyperparam_grid', client)
...
def train_model(hyperparam_idx: int, retcode_download: int, N_gridsize: int) -> int:
    ...
    features_train = torch.from_numpy(utils.read_from_store(bucket_name, 'features_train', client)).float()
    target_train = torch.from_numpy(utils.read_from_store(bucket_name, 'target_train', client)).float()
    features_test = torch.from_numpy(utils.read_from_store(bucket_name, 'features_test', client)).float()
    target_test = torch.from_numpy(utils.read_from_store(bucket_name, 'target_test', client)).float()

    conf = utils.read_from_store(config.BUCKET_NAME, 'hyperparam_grid', client)[hyperparam_idx]
    ...
    utils.write_to_store(bucket_name, {'test_loss': test_loss.item(), 'model': model}, f'score_{hyperparam_idx}', client)
...
def find_best(N_experiments: int) -> int:
    ...
    grid = utils.read_from_store(bucket_name, 'hyperparam_grid', client)

    results = []
    for i in range(N_experiments):
        results.append(utils.read_from_store(config.BUCKET_NAME, f'score_{i}', client))
...
def list_results():
    ...
    grid = utils.read_from_store(config.BUCKET_NAME, 'hyperparam_grid', client)
    for i in range(len(grid)): 
        print(grid[i], utils.read_from_store(config.BUCKET_NAME, f'score_{i}', client)['test_loss']) 
...
def run_pipeline():
    ...
    retcode_download = download_data()
    N_gridsize = gen_hyperparam_grid()

    retcode_list = []
    for i in range(N_gridsize):
        retcode_model = train_model(i, retcode_download, N_gridsize)
        retcode_list.append(retcode_model)

    best_idx = find_best(len(retcode_list))
