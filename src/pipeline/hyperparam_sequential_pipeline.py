...
def generate_random_search_point(iter_val: int) -> int:
    ...
    conf['num_hidden_layers'] = max(conf['num_hidden_layers'] + (np.random.randint(3)-1), 0)
...
def print_gen_val(iter_val: int) -> int:
    ...
    for i in range(iter_val+1):
        conf = read_from_store(bucket_name, f'conf_{i}', client)
        print(i)
        print(conf)
        print('-----------')
...
def train_model(hyperparam_idx: int, retcode_download: int) -> int:
...
def run_pipeline():
    ...
    for i in list(range(10)): #try to be as close as possible to canonical python for DS people
        ...
        #retcode_model = train_model_op(i, retcode_download.output)
